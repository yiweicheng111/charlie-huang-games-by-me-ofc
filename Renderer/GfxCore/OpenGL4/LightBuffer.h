...
namespace Cle::OPENGL {
	...
	public:
		GLuint lightTexture;
		float sizeDirty = false;
		float dataDirty = false;
		GLuint m_Program;
		...
		LightBuffer(GLuint Program) : m_Program(Program) 
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, 2, LightFloatVector.size(), 0, GL_RGBA32F, GL_FLOAT, LightFloatVector.data());
		}
		...
		void updateLight(int index, Cle::Gfx::Light light)
		{
			...
			LightFloatVector[foffset +5] = light.position.x;
			LightFloatVector[foffset +6] = light.position.x;
		...
		void updateVector()
		{
			...
			if (dataDirty)
			{
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 2, LightFloatVector.size(), GL_RGBA, GL_FLOAT, LightFloatVector.data());
				dataDirty = false;
			}
		}
	};
